...
`RAW_DELETE` e `RAW_ATTRIB`. `app.c` converte `ALFRED_RAW_CREATE`,
`ALFRED_RAW_DELETE` e `ALFRED_RAW_ATTRIB` con `alfred_record_from_raw()`, li
invia al sink comune, scrive il payload normalizzato su `raw.log` e poi passa
comunque il raw originale ad `alfred_process()`. I diagnostici runtime

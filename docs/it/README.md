...
| [46 - Metadata/session record JSONL v0](46-metadata-session-record-jsonl-v0.md) | Contratto del record `SESSION_CONTEXT`: payload JSONL `workspace.root`, `workspace.id`, `ledger.session_id`, omissione dei campi vuoti e divieto di per-event enrichment accidentale. L'emissione runtime resta un passo successivo. | Chi lavora su JSONL, output pipeline, ledger observe-mode e contratti workspace/sessione. | Dopo il contesto runtime e prima di collegare il record metadata/sessione alla pipeline runtime. |

...
| [46 - Metadata/session record JSONL v0](46-metadata-session-record-jsonl-v0.md) | Roadmap per pubblicare il contesto workspace/sessione come record metadata/sessione JSONL separato, senza arricchire ogni evento. | Chi lavora su JSONL, output pipeline, ledger observe-mode e contratti workspace/sessione. | Dopo il contesto runtime e prima di esporre quei campi in JSONL pubblico. |

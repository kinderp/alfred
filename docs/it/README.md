...
| [44 - Workspace/session runtime schema v0](44-workspace-session-runtime-schema-v0.md) | Milestone per decidere significato, fonte di verita', ownership, JSONL, test e benchmark gate di `workspace_root`, `workspace_id` e `ledger_session_id`. | Maintainer, contributori security, agenti AI e chi lavora sul modello record/runtime. | Dopo il ledger observe-mode e prima di implementare campi runtime workspace/sessione. |
| [45 - Workspace/session runtime context v0](45-workspace-session-runtime-context-v0.md) | Primo micro-step di codice per salvare `workspace_root`, `workspace_id` e `ledger_session_id` in un contesto app-owned senza cambiare record o JSONL. | Chi lavora su configurazione, runtime app e futuri campi workspace/sessione. | Prima di usare o modificare il contesto runtime workspace/sessione. |
| [46 - Metadata/session record JSONL v0](46-metadata-session-record-jsonl-v0.md) | Contratto del record `SESSION_CONTEXT`: payload JSONL `workspace.root`, `workspace.id`, `ledger.session_id`, omissione dei campi vuoti, emissione runtime one-shot tramite output pipeline e divieto di per-event enrichment accidentale. | Chi lavora su JSONL, output pipeline, ledger observe-mode e contratti workspace/sessione. | Dopo il contesto runtime e quando si tocca l'emissione metadata/sessione nella pipeline runtime. |
| [47 - Inotify reference backend MVP closure audit](47-inotify-reference-backend-mvp-closure-audit.md) | Roadmap per auditare e chiudere il backend Linux inotify come primo backend di riferimento dell'MVP, separando supporto attuale, test, debiti e roadmap futura. | Maintainer, contributori backend, agenti AI. | Dopo la chiusura delle milestone JSONL/sessione e prima di aprire nuovi backend o Agent Guard. |
| [48 - Core Input Model / Main Loop Migration v0](48-core-input-main-loop-migration-v0.md) | Decisione sul modello di input del core dopo Backend API v0 staged: raw-first, bridge misurato, record-first, benchmark gate e criteri di riapertura. | Maintainer, contributori core/backend e agenti AI. | Dopo la chiusura MVP del backend inotify e prima di riaprire il percorso caldo. |
| [49 - MVP operational usability v0](49-mvp-operational-usability-v0.md) | Roadmap per rendere l'MVP corrente piu' usabile: CLI minima, README/man page, smoke test, log/output e readiness operativa. | Maintainer, contributori, utenti tecnici e agenti AI. | Dopo la decisione raw-first e prima di allargare Alfred con nuovi backend o Agent Guard. |
| [50 - CLI parser v0](50-cli-parser-v0.md) | Roadmap della milestone per introdurre un parser CLI minimo: `-c`, `--config`, `--`, decisione su `--print-config`, precedenza config e test CLI. | Chi lavora su CLI, config, README, man page e test utente. | Dopo MVP operational usability v0 e prima di modificare parsing argomenti o documentazione CLI. |
| [51 - Post-MVP documentation and man pages v0](51-post-mvp-documentation-man-pages-v0.md) | Roadmap per riallineare README, README italiano, pagine man inglesi/italiane e documentazione didattica dopo la stabilizzazione del parser CLI. | Maintainer, contributori, utenti tecnici e agenti AI. | Dopo CLI parser v0 e prima di ampliare man page, README o documentazione pubblica post-MVP. |
| [52 - Configurazione Codex Sol Ultra](52-configurazione-codex-sol-ultra.md) | Profilo Codex sanificato per `gpt-5.6-sol`, reasoning `ultra`, multi-agent e script di installazione in `~/.codex`. | Maintainer, studenti e agenti AI che devono replicare l'ambiente Codex su un'altra macchina. | Quando bisogna configurare una nuova workstation Codex per lavorare su Alfred. |
| [53 - Installability and Linux compatibility v0](53-installability-linux-compatibility-v0.md) | Contratto e roadmap per installazione staged, layout posseduto da Alfred, matrice Linux userspace e futura copertura di kernel reali senza confondere container e VM. | Maintainer, contributori packaging/CI, studenti e agenti AI. | Prima di cambiare install/uninstall, lane userspace, dichiarazioni di compatibilita' o future prove VM/kernel. |
| [54 - Audit installazione e assunzioni CI v0](54-audit-installazione-ci-v0.md) | Fotografia storica verificata di artefatti debug/release, man page installabili, dipendenze delle suite e assunzioni CI prima del contratto staged. | Maintainer, contributori packaging/CI, studenti e reviewer del Makefile. | Per capire perche' il contratto install e la matrice corrente sono separati per profilo, dipendenze e livello di evidenza. |
| [55 - First-user pre-release validation v0](55-first-user-pre-release-validation-v0.md) | Protocollo per consenso, onboarding, scenari reali, evidenza, privacy, triage e criteri di uscita della prima validazione con utenti esterni. | Partecipanti, facilitatori, maintainer, studenti e reviewer. | Prima di organizzare, eseguire o valutare una sessione pre-release con un primo utente. |
| [README pubblico italiano](../../README.it.md) | Traduzione italiana del README pubblico dell'MVP corrente. | Utenti tecnici, studenti e nuovi contributori italiani. | Quando si vuole provare Alfred senza leggere subito il README inglese. |
...
Le traduzioni italiane sono in layout locale sotto `docs/man/it/`:

- [alfred(1) italiano](../man/it/man1/alfred.1)
- [alfred.conf(5) italiano](../man/it/man5/alfred.conf.5)
- [alfred-events(7) italiano](../man/it/man7/alfred-events.7)

...
| Caricare configurazione da file | `ALFRED_CONFIG=FILE`; `-c FILE`; `--config FILE` | CLI esplicita vince su `ALFRED_CONFIG`: se `-c`/`--config` e' presente, il file ambiente non viene caricato | Supportato in CLI parser v0 |
...
| Validare configurazione senza runtime | `./alfred --check-config`, `./alfred -c FILE --check-config`, `./alfred --config FILE --check-config`, con eventuale fallback `ALFRED_CONFIG=FILE` | Non richiedere `PATH...`; rifiutare path o comandi combinati; non creare log runtime | Supportato in CLI parser v0 |

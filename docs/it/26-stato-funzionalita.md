...
| Golden test JSONL end-to-end | `make test-jsonl` | Primi scenari supportati | Verifica `output.jsonl` con parsing JSON reale per create file, create directory, modify/close-write file, attrib raw, delete file, delete directory, `WATCH_ADDED`, `WATCH_REMOVED`, rename file, move file, relocated file, rename directory, move directory, directory relocated, raw move cookie, `RAW_ATTRIB`, `FILE_MODIFIED`, `FILE_READY`, `FILE_DELETED`, `DIR_DELETED`, `FILE_RENAMED`, `FILE_MOVED`, `FILE_RELOCATED`, `DIR_RENAMED`, `DIR_MOVED`, `DIR_RELOCATED` e diagnostica recovery base `WATCH_STALE`/`WATCH_RESYNC_FAILED`/`WATCH_LOST_QUEUED`; da estendere a recovery completa ed errori |

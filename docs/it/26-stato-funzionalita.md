...
| Backend API v0 | Documentata, primo tipo record, adapter raw, adapter semantico, builder diagnostico, formatter testuale e primo log runtime `WATCH_ADDED` via record implementati | Specifica in `30-backend-api-v0.md`; manca refactor inotify a `emit(record)` e migrazione completa degli altri `WATCH_*` |

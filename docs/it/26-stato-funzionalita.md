...
| --- | --- | --- |
| CLI operativa | Path posizionali `PATH...`; configurazione tramite `ALFRED_CONFIG`; comandi informativi `--help` e `--version` che terminano prima di configurazione/logger/backend/core/output e non creano log runtime; `--check-config` valida default, `ALFRED_CONFIG` e `ALFRED_EVENT_ENGINE` senza avviare logger/backend/core/output/watch | parser completo, `-c`/`--config`, `--print-config`, `--`, usage amichevole per opzioni sconosciute; validazione dei path fuori da `--check-config` |

...
| Backend state | watch ricorsivi, `WATCH_ADDED`, `WATCH_REMOVED`, `WATCH_STALE`, resync locale, lost-scope queue, recovery delayed sincrona, retry/backoff, benchmark manuale lost-scope; builder iniziale verso record diagnostici `WATCH_*`; `WATCH_ADDED`, `WATCH_REMOVED`, `WATCH_STALE` e `WATCH_RESYNC_FAILED` logico runtime via record + formatter | worker thread, configurazione pubblica recovery, performance suite stabile, backpressure, migrazione degli altri record diagnostici runtime |
...
| Backend API v0 | Documentata, primo tipo record, adapter raw, adapter semantico, builder diagnostico, formatter testuale e primi log runtime `WATCH_ADDED`/`WATCH_REMOVED`/`WATCH_STALE`/`WATCH_RESYNC_FAILED` logico via record implementati; policy OS error documentata; campi OS error presenti in `alfred_record_t`; builder diagnostico con OS error e formatter compatibile `errno=N (...)` disponibili | Specifica in `30-backend-api-v0.md`; manca refactor inotify a `emit(record)`, uso runtime dei campi OS error e migrazione completa degli altri `WATCH_*` |

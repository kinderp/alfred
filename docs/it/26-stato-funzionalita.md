...
| --- | --- | --- |
| CLI operativa | Path posizionali `PATH...`; configurazione tramite `ALFRED_CONFIG`; comandi informativi `--help` e `--version` che terminano prima di configurazione/logger/backend/core/output e non creano log runtime | parser completo, `-c`/`--config`, `--check-config`, `--print-config`, `--`, usage amichevole per opzioni sconosciute |

...
| --- | --- | --- |
| JSONL runtime stabile | Parziale/rimandato | Formatter, sink, writer buffered, configurazione output e golden rappresentativi esistono; lifecycle/app resta non-goal JSONL v0 e rimane nei log umani; i failure writer/output sono fail-closed e restano in `errors.log`/exit status, non in record JSONL generici; mancano worker asincrono, backpressure e un eventuale modello pubblico futuro per lifecycle/error |

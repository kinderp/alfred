...
| Output pipeline single-writer | `alfred_record_output_pipeline_t` | Collegata in modo sincrono dietro configurazione | Compone queue, dispatcher, runtime drain e JSONL writer per raw record normalizzati, eventi semantici core e diagnostica watch `WATCH_ADDED`/`WATCH_REMOVED`/`WATCH_STALE` |

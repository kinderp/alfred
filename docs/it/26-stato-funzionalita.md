...
| Backend state | watch ricorsivi, `WATCH_ADDED`, `WATCH_REMOVED`, `WATCH_STALE`, resync locale, lost-scope queue, recovery delayed sincrona, retry/backoff, benchmark manuale lost-scope; builder iniziale verso record diagnostici `WATCH_*`; `WATCH_ADDED`, `WATCH_REMOVED`, `WATCH_STALE`, `WATCH_RESYNC_*` e `WATCH_LOST_*` runtime via record + formatter | worker thread, configurazione pubblica recovery, performance suite stabile, backpressure, sink comune `emit(record)` |
...
| Backend API v0 | Documentata, primo tipo record, adapter raw, adapter semantico, builder diagnostico, formatter testuale e log runtime `WATCH_ADDED`/`WATCH_REMOVED`/`WATCH_STALE`/`WATCH_RESYNC_*`/`WATCH_LOST_*` via record implementati; policy OS error documentata; campi OS error presenti in `alfred_record_t`; runtime `WATCH_RESYNC_FAILED` con `errno=N (...)` via record disponibile | Specifica in `30-backend-api-v0.md`; manca refactor inotify a `emit(record)` e migrazione del raw path runtime |

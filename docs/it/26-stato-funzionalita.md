...
| Runtime output benchmark | `make perf-runtime-output` | Prima baseline runtime reale supportata | Misura backend inotify reale, callback app, log compatibili e output JSONL opt-in su file temporanei; il CSV include anche contatori locali della coda come enqueue, pressure drains, drain calls, drained records e max pending |

...
| Golden test JSONL end-to-end | `make test-jsonl` | Primi scenari supportati | Verifica `output.jsonl` con parsing JSON reale per create file, create directory, `WATCH_ADDED`, rename file, rename directory, directory relocated, raw move cookie, `FILE_RENAMED`, `DIR_RENAMED`, `DIR_RELOCATED` e diagnostica recovery base `WATCH_STALE`/`WATCH_RESYNC_FAILED`/`WATCH_LOST_QUEUED`; da estendere ad altri scenari move/relocate, recovery completa ed errori |

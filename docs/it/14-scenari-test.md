...
| watch stale output failure | `tests/backend/test_watch_stale_output_failure.sh` + `tests/backend/test_watch_stale_output_failure.c` | `backend_log_watch_stale()` con callback `emit_record` riuscito/fallito; path lungo che forza fallback legacy del text sink; `events.log` contiene comunque `WATCH_STALE ... reason=IN_MOVE_SELF`; `errors.log` contiene `failed to emit watch stale output record`; helper ritorna errore quando il callback fallisce | verifica il ramo di poll dei self-event: `WATCH_STALE` non nasce da `watch_manager_add()`, ma da `IN_MOVE_SELF`/`IN_DELETE_SELF`/`IN_UNMOUNT`; se il record strutturato non puo' attraversare la callback, il backend deve propagare errore invece di continuare con un ledger incompleto; il caso path lungo prova che il fallback di `events.log` non salta `emit_record` |

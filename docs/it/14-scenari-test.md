...
| watch stale output failure | `tests/backend/test_watch_stale_output_failure.sh` + `tests/backend/test_watch_stale_output_failure.c` | `backend_log_watch_stale()` con callback `emit_record` che fallisce solo sul `WATCH_STALE`; `events.log` contiene comunque `WATCH_STALE ... reason=IN_MOVE_SELF`; `errors.log` contiene `failed to emit watch stale output record`; helper ritorna errore | verifica il ramo di poll dei self-event: `WATCH_STALE` non nasce da `watch_manager_add()`, ma da `IN_MOVE_SELF`/`IN_DELETE_SELF`/`IN_UNMOUNT`; se il record strutturato non puo' attraversare la callback, il backend deve propagare errore invece di continuare con un ledger incompleto |
| resync output routing | `tests/backend/test_resync_output_routing.sh` + `tests/backend/test_resync_output_routing.c` | `WATCH_RESYNC_BEGIN`, `WATCH_RESYNC_FAILED`, callback `emit_record` riuscito, callback fallito, path lungo che forza fallback legacy del text sink | verifica il confine strutturato dei diagnostici resync: il backend scrive prima il log compatibile, poi offre lo stesso `alfred_record_t` borrowed a `emit_record`; se il callback fallisce il helper torna errore per permettere la policy fail-closed; il caso path lungo prova che JSONL non dipende dal successo del formatter testuale umano |

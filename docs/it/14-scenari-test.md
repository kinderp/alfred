...
| output config | `tests/backend/test_output_config.sh` + `tests/backend/test_output_config.c` | default `output_enabled=false`, `output_format=jsonl`, `output_buffer_size=65536`, `output_log=output.jsonl`; accettazione `text`/`jsonl`; rifiuto di formato non implementato e buffer invalido | verifica la configurazione del writer runtime senza avviare Alfred; `text` resta accettabile come valore configurato ma non e' ancora attivabile in runtime |
| record output pipeline | `tests/backend/test_record_output_pipeline.sh` + `tests/backend/test_record_output_pipeline.c` | pipeline disabilitata no-op; pipeline JSONL abilitata; enqueue owned; drain batch; queue full; flush failure preservato | verifica il primo assemblaggio testabile `record -> queue -> dispatcher -> JSONL writer` isolato da ownership di file e runtime applicativo |

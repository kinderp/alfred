...
| output pipeline runtime | `tests/backend/test_output_pipeline_runtime.sh` | `ALFRED_CONFIG` con `output_enabled=true`, creazione file e directory, creazione/rimozione di una directory watched, `raw.log` ed `events.log` compatibili ancora presenti, `output.jsonl` con record `RAW_CREATE`, `FILE_CREATED`, `DIR_CREATED`, `WATCH_ADDED`, `WATCH_STALE`, `WATCH_STALE_EVENT_DROPPED` e `WATCH_REMOVED` | verifica il collegamento di `alfred_record_output_pipeline_t` ad `app_run()` in modo sincrono e aggiuntivo per raw normalizzati, eventi semantici core e diagnostica watch base |

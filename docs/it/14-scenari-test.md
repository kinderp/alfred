...
| output pipeline runtime | `tests/backend/test_output_pipeline_runtime.sh` | `ALFRED_CONFIG` con `output_enabled=true`, creazione file e directory, `raw.log` ed `events.log` compatibili ancora presenti, `output.jsonl` con record `RAW_CREATE`, `FILE_CREATED` e `DIR_CREATED` | verifica il collegamento di `alfred_record_output_pipeline_t` ad `app_run()` in modo sincrono e aggiuntivo per raw normalizzati ed eventi semantici core |

...
| lost scope recovery | `tests/backend/test_lost_scope_recovery.sh` + `tests/backend/test_lost_scope_recovery.c` | `WATCH_LOST_SCAN_BEGIN`, `WATCH_LOST_FOUND`, `WATCH_LOST_NOT_FOUND` senza mutazioni watcher table | verifica la recovery sincrona read-only: una directory rinominata viene ritrovata per `(st_dev, st_ino)` dentro una root monitorata, mentre una directory cancellata non viene trovata e non produce eventi semantici |

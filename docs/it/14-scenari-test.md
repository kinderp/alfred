...
| lost scope recovery | `tests/backend/test_lost_scope_recovery.sh` + `tests/backend/test_lost_scope_recovery.c` | `WATCH_LOST_SCAN_BEGIN`, `WATCH_LOST_FOUND`, `WATCH_LOST_PREFIX_UPDATED`, `WATCH_LOST_COVERAGE_DONE`, `WATCH_LOST_COVERAGE_MISSING`, `WATCH_LOST_COVERAGE_CLASS`, `WATCH_LOST_REINSTALLED`, `WATCH_LOST_RECOVERY_END`, `WATCH_LOST_REINSTALL_FAILED`, `WATCH_LOST_ROLLBACK`, `WATCH_LOST_RECOVERY_FAILED`, `WATCH_LOST_NOT_FOUND`, processore due che non consuma entry future | verifica la recovery sincrona completa: il ramo positivo ritrova una directory per `(st_dev, st_ino)`, riallinea path, reinstalla un missing watch e marca la subtree `VALID`; il ramo failure forza il secondo fake add a fallire, rimuove il primo fake watch installato nello stesso tentativo e lascia root/child `STALE`; una directory cancellata non viene trovata; una entry con `retry_after_ns` futuro resta in queue senza log |

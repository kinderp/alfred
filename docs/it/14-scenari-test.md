...
| lost scope recovery | `tests/backend/test_lost_scope_recovery.sh` + `tests/backend/test_lost_scope_recovery.c` | `WATCH_LOST_SCAN_BEGIN`, `WATCH_LOST_FOUND`, `WATCH_LOST_NOT_FOUND`, aggiornamento del path principale senza ritorno a `VALID` | verifica la recovery sincrona parziale: una directory rinominata viene ritrovata per `(st_dev, st_ino)` dentro una root monitorata e aggiorna solo il path del watch principale, mentre una directory cancellata non viene trovata e non produce eventi semantici |

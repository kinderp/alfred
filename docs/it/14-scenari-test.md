...
| lost scope recovery | `tests/backend/test_lost_scope_recovery.sh` + `tests/backend/test_lost_scope_recovery.c` | `WATCH_LOST_SCAN_BEGIN`, `WATCH_LOST_FOUND`, `WATCH_LOST_PREFIX_UPDATED`, `WATCH_LOST_COVERAGE_DONE`, `WATCH_LOST_COVERAGE_MISSING`, `WATCH_LOST_COVERAGE_CLASS`, `WATCH_LOST_NOT_FOUND`, aggiornamento del path principale e di un figlio gia' noto senza ritorno a `VALID` | verifica la recovery sincrona parziale: una directory rinominata viene ritrovata per `(st_dev, st_ino)`, riallinea i path gia' presenti nella watcher table e misura una directory figlia non watched come `missing=1`; una directory cancellata non viene trovata e non produce eventi semantici |
...
| Scenario | Script | Cosa controlla | Perche' serve |
| --- | --- | --- | --- |
| watcher reliability states | `tests/watcher/test_watcher_state.c` | store iniziale `VALID`, identita' `st_dev/st_ino`, `watcher_update_path()` che preserva stato/identita', `watcher_update_path_prefix()` che riscrive una subtree preservando stato/identita' e ignorando prefissi simili ma esterni, transizioni a `STALE` e `RESYNCING`, remove a `REMOVED`, rifiuto di `REMOVED` su slot attivo, conteggio per stato, iterazione per stato e stop anticipato | prepara la futura gestione `IN_MOVE_SELF` e resync senza dipendere da timing o dettagli del kernel |

...
| self move identity match | `tests/backend/test_self_move_identity_match.sh` | `IN_MOVE_SELF`, path osservato ripristinato con la stessa identita', `WATCH_RESYNC_SCAN_DONE ... dirs=2 watched=1 missing=1`, `WATCH_RESYNC_SCAN_CLASS ... result=needs-reinstall`, `WATCH_RESYNC_END ... result=valid` | verifica il ramo positivo del probe: se la root osservata viene spostata e rimessa nello stesso path prima del resync, `(st_dev, st_ino)` coincidono, Alfred esegue lo scan dry-run directory-only, misura copertura watch gia' presente/mancante, classifica il risultato come read-only e il watch puo' tornare `VALID` |

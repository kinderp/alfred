...
| lost-scope queue output routing | `tests/backend/test_lost_scope_queue_output_routing.sh` + `tests/backend/test_lost_scope_queue_output_routing.c` | `WATCH_LOST_QUEUED`, `WATCH_LOST_QUEUE_FAILED`, callback `emit_record` riuscito e callback fallito | verifica il primo confine strutturato lost-scope: il backend scrive prima il log compatibile, poi offre i record queue-level a `emit_record`; i record di scan/recovery lost-scope restano fuori da questo micro-step |
| lost-scope scan output routing | `tests/backend/test_lost_scope_scan_output_routing.sh` + `tests/backend/test_lost_scope_scan_output_routing.c` | `WATCH_LOST_SCAN_BEGIN`, `WATCH_LOST_FOUND`, `WATCH_LOST_PREFIX_UPDATED`, `WATCH_LOST_COVERAGE_DONE`, `WATCH_LOST_COVERAGE_MISSING`, `WATCH_LOST_COVERAGE_CLASS`, `WATCH_LOST_NOT_FOUND`, `WATCH_LOST_RECOVERY_FAILED`, callback `emit_record` riuscito e callback fallito | verifica il confine strutturato della fase di ricerca e classificazione lost-scope: old/new path, contatori di coverage, missing path, retry count ed error token vengono preservati nel record; reinstallazione, rollback, retry scheduling e recovery end restano il micro-step successivo |

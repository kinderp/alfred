...
| output pipeline runtime | `tests/backend/test_output_pipeline_runtime.sh` | `ALFRED_CONFIG` con `output_enabled=true`, creazione file e directory, creazione/rimozione di una directory watched, spostamento di una directory watched che produce resync fallito, `raw.log` ed `events.log` compatibili ancora presenti, `output.jsonl` con record `RAW_CREATE`, `FILE_CREATED`, `DIR_CREATED`, `WATCH_ADDED`, `WATCH_STALE`, `WATCH_STALE_EVENT_DROPPED`, `WATCH_REMOVED`, `WATCH_RESYNC_BEGIN` e `WATCH_RESYNC_FAILED`; `/dev/full` usato sia per fallimento durante il loop sia per fallimento solo al flush finale | verifica il collegamento di `alfred_record_output_pipeline_t` ad `app_run()` in modo sincrono e aggiuntivo per raw normalizzati, eventi semantici core, diagnostica watch base e diagnostica resync; verifica anche che un ledger JSONL incompleto faccia uscire Alfred con stato non-zero anche quando l'errore emerge solo nello shutdown |

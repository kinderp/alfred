...
| lost scope recovery | `tests/backend/test_lost_scope_recovery.sh` + `tests/backend/test_lost_scope_recovery.c` | `WATCH_LOST_SCAN_BEGIN`, `WATCH_LOST_FOUND`, `WATCH_LOST_PREFIX_UPDATED`, `WATCH_LOST_COVERAGE_DONE`, `WATCH_LOST_COVERAGE_MISSING`, `WATCH_LOST_COVERAGE_CLASS`, `WATCH_LOST_REINSTALLED`, `WATCH_LOST_RECOVERY_END`, `WATCH_LOST_NOT_FOUND`, aggiornamento path, reinstallazione fake di un missing watch e ritorno a `VALID` | verifica la recovery sincrona completa positiva: una directory rinominata viene ritrovata per `(st_dev, st_ino)`, riallinea i path gia' presenti nella watcher table, misura una directory figlia non watched come `missing=1`, reinstalla quel watch con fake ops deterministiche e marca la subtree `VALID`; una directory cancellata non viene trovata e non produce eventi semantici |

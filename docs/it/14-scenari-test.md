...
| lost scope recovery | `tests/backend/test_lost_scope_recovery.sh` + `tests/backend/test_lost_scope_recovery.c` | `WATCH_LOST_SCAN_BEGIN`, `WATCH_LOST_FOUND`, `WATCH_LOST_PREFIX_UPDATED`, `WATCH_LOST_NOT_FOUND`, aggiornamento del path principale e di un figlio gia' noto senza ritorno a `VALID` | verifica la recovery sincrona parziale: una directory rinominata viene ritrovata per `(st_dev, st_ino)` dentro una root monitorata e riallinea i path gia' presenti nella watcher table, mentre una directory cancellata non viene trovata e non produce eventi semantici |

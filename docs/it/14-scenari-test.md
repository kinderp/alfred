...
| lost scope queue | `tests/backend/test_lost_scope_queue.sh` + `tests/backend/test_lost_scope_queue.c` | helper statici `backend_lost_scope_queue_*()` senza log runtime, incluso `peek` read-only della testa FIFO | verifica la struttura dati che prepara la recovery posticipata: lifecycle, ordine FIFO, crescita del buffer circolare, copia stabile di path/scan_root/reason, controllo della maturita' senza pop e rifiuto di input invalidi |

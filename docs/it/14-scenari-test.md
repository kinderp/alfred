...
| resync output routing | `tests/backend/test_resync_output_routing.sh` + `tests/backend/test_resync_output_routing.c` | `WATCH_RESYNC_BEGIN`, `WATCH_RESYNC_FAILED`, callback `emit_record` riuscito e callback fallito | verifica il confine strutturato dei diagnostici resync: il backend scrive prima il log compatibile, poi offre lo stesso `alfred_record_t` borrowed a `emit_record`; se il callback fallisce il helper torna errore per permettere la policy fail-closed |

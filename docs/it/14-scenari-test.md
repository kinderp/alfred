...
| output pipeline runtime | `tests/backend/test_output_pipeline_runtime.sh` | `ALFRED_CONFIG` con `output_enabled=true`, creazione file e directory, `raw.log` compatibile ancora presente, `output.jsonl` con record `RAW_CREATE` | verifica il primo collegamento di `alfred_record_output_pipeline_t` ad `app_run()` in modo sincrono e aggiuntivo |

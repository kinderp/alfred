...
| resync reinstall policy | `tests/backend/test_resync_reinstall_policy.sh` + `tests/backend/test_resync_reinstall_policy.c` | helper statico `backend_resync_reinstall_missing_watches()` con fake add/remove | verifica senza race filesystem la policy all-or-stale: se un missing watch fallisce dopo una reinstallazione riuscita, Alfred rimuove i watch installati in quel tentativo e ritorna `ERR_INOTIFY` |

...
| watch mask unmount token | `tests/backend/test_watch_mask_unmount_token.sh` | avvio riuscito con `inotify_watch_mask=IN_UNMOUNT,IN_IGNORED` | verifica che `IN_UNMOUNT` sia un token supportato dal backend. Il test non genera un vero unmount perche' richiederebbe privilegi e un ambiente di mount controllato |
| audit kernel events | `tests/backend/test_audit_kernel_events.sh` + `tests/backend/test_audit_kernel_events.c` | watch inotify diretto con `IN_OPEN`, `IN_ACCESS`, `IN_CLOSE_NOWRITE`; apertura, lettura e chiusura read-only di un file; assenza di `IN_CLOSE_WRITE` | test osservativo sul kernel, non sul runtime Alfred. Fissa che gli eventi audit sono reali, ma restano fuori da raw/core Alfred finche' non avremo una policy audit esplicita |
| audit config raw log | `tests/backend/test_audit_config_raw_log.sh` | `inotify_audit_events=open,access,close-nowrite`, lettura read-only, raw log con `IN_OPEN`, `IN_ACCESS`, `IN_CLOSE_NOWRITE`, nessun `FILE_READY`/`FILE_MODIFIED` | verifica il primo contratto runtime opt-in: Alfred puo' chiedere eventi audit al kernel e renderli nel raw log senza promuoverli a semantica core |
| audit config invalid token | `tests/backend/test_audit_config_invalid_token.sh` | avvio fallito con `inotify_audit_events=IN_OPEN` | verifica che la configurazione audit usi nomi policy (`open`, `access`, `close-nowrite`) e non token raw `IN_*` |
| output config | `tests/backend/test_output_config.sh` + `tests/backend/test_output_config.c` | default `output_enabled=false`, `output_format=jsonl`, `output_buffer_size=65536`, `output_log=output.jsonl`; accettazione `text`/`jsonl`/`counter`; rifiuto di formato non implementato e buffer invalido | verifica la configurazione del writer runtime senza avviare Alfred; `text` resta accettabile come valore configurato ma non e' ancora attivabile in runtime, mentre `counter` e' un formato benchmark/no-op per misurare queue e dispatcher senza JSONL |
| record output pipeline | `tests/backend/test_record_output_pipeline.sh` + `tests/backend/test_record_output_pipeline.c` | pipeline disabilitata no-op; pipeline JSONL abilitata; pipeline counter abilitata senza output; enqueue owned; drain batch; queue full; flush failure preservato | verifica il primo assemblaggio testabile `record -> queue -> dispatcher -> sink` isolato da ownership di file e runtime applicativo; il caso JSONL misura il writer reale, il caso counter misura lo stesso confine senza formattazione, buffering o I/O |
| output counter runtime | `tests/backend/test_output_counter_runtime.sh` | `ALFRED_CONFIG` con `output_enabled=true`, `output_format=counter`, eventi reali da file e directory, `raw.log` ed `events.log` compatibili ancora presenti, riga `output runtime stats`, `enqueue_attempts>0`, `enqueue_attempts=enqueue_success=drained_records`, `enqueue_failures=0`, assenza del file `output_log` | verifica che il percorso runtime counter attraversi davvero `record -> queue -> drain -> dispatcher -> counter sink` senza produrre JSONL e senza usare il benchmark manuale come unico oracolo di correttezza |
| output pipeline runtime | `tests/backend/test_output_pipeline_runtime.sh` | `ALFRED_CONFIG` con `output_enabled=true`, creazione file e directory, creazione/rimozione di una directory watched, spostamento di una directory watched che produce resync fallito e accodo lost-scope, `raw.log` ed `events.log` compatibili ancora presenti, `output.jsonl` con record `RAW_CREATE`, `FILE_CREATED`, `DIR_CREATED`, `WATCH_ADDED`, `WATCH_STALE`, `WATCH_STALE_EVENT_DROPPED`, `WATCH_REMOVED`, `WATCH_RESYNC_BEGIN`, `WATCH_RESYNC_FAILED` e `WATCH_LOST_QUEUED`; `/dev/full` usato sia per fallimento durante il loop sia per fallimento solo al flush finale; riga `output runtime stats` in `events.log` | verifica il collegamento di `alfred_record_output_pipeline_t` ad `app_run()` in modo sincrono e aggiuntivo per raw normalizzati, eventi semantici core, diagnostica watch base, diagnostica resync e handoff queue-level lost-scope; dopo lo split Writer Runtime v0 copre il confine `app_emit_output_record() -> app_enqueue_output_record()`, il drain esplicito `app_run() -> app_drain_output_pipeline()`, la valvola di backpressure quando una burst riempie la coda bounded e i contatori locali del runtime output; verifica che un ledger JSONL incompleto faccia uscire Alfred con stato non-zero sia quando l'errore emerge durante il drain/write sia quando emerge solo nello shutdown |
| output queue pressure | `tests/backend/test_output_queue_pressure.sh` | burst di file con `output_enabled=true`, riga `output runtime stats`, `enqueue_attempts=enqueue_success`, `enqueue_failures=0`, `pressure_drains>0`, `drained_records=enqueue_success`, `max_pending` che raggiunge la capacita' bounded corrente, righe JSONL pari ai record drenati | verifica in modo mirato la valvola di pressione v0: se un singolo poll backend produce piu' record di quanti la coda puo' tenere prima del drain ordinario di `app_run()`, `app_enqueue_output_record()` drena una volta e ritenta invece di trasformare una burst legittima in errore output |
| watch diagnostic output failure | `tests/backend/test_watch_diagnostic_output_failure.sh` + `tests/backend/test_watch_diagnostic_output_failure.c` | `watch_manager_add()` con callback `emit_record` che fallisce sul `WATCH_ADDED`; rollback del watch appena installato; chiamata mirata al helper watch manager con path lungo che forza fallback legacy del text sink; `errors.log` con failure diagnostico | verifica il contratto fail-closed per diagnostica backend: il backend non conosce JSONL, ma deve propagare il fallimento della callback applicativa invece di lasciare Alfred con un ledger strutturato incompleto; il caso path lungo prova che `WATCH_ADDED` attraversa comunque `emit_record` anche quando `events.log` viene scritto tramite fallback legacy |
| watch stale output failure | `tests/backend/test_watch_stale_output_failure.sh` + `tests/backend/test_watch_stale_output_failure.c` | `backend_log_watch_stale()` e `backend_log_stale_event_dropped()` con callback `emit_record` riuscito/fallito; path lungo che forza fallback legacy del text sink; `events.log` contiene comunque `WATCH_STALE ... reason=IN_MOVE_SELF` e `WATCH_STALE_EVENT_DROPPED ... mask=IN_IGNORED name=after-move.txt`; `errors.log` contiene `failed to emit watch stale output record` e `failed to emit stale dropped output record`; i helper tornano errore quando il callback fallisce | verifica il ramo di poll dei self-event: `WATCH_STALE` non nasce da `watch_manager_add()`, ma da `IN_MOVE_SELF`/`IN_DELETE_SELF`/`IN_UNMOUNT`; `WATCH_STALE_EVENT_DROPPED` nasce quando il kernel invia ancora un evento su un watch stale e Alfred lo droppa invece di inventare un raw/core event con path inaffidabile; se il record strutturato non puo' attraversare la callback, il backend deve propagare errore invece di continuare con un ledger incompleto; il caso path lungo prova che il fallback di `events.log` non salta `emit_record` |
| resync output routing | `tests/backend/test_resync_output_routing.sh` + `tests/backend/test_resync_output_routing.c` | `WATCH_RESYNC_BEGIN`, `WATCH_RESYNC_FAILED`, callback `emit_record` riuscito, callback fallito, path lungo che forza fallback legacy del text sink sia nel helper ricco sia nel helper generico `WATCH_RESYNC_FAILED` | verifica il confine strutturato dei diagnostici resync: il backend scrive prima il log compatibile, poi offre lo stesso `alfred_record_t` borrowed a `emit_record`; se il callback fallisce il helper torna errore per permettere la policy fail-closed; il caso path lungo prova che JSONL non dipende dal successo del formatter testuale umano |
| lost-scope queue output routing | `tests/backend/test_lost_scope_queue_output_routing.sh` + `tests/backend/test_lost_scope_queue_output_routing.c` | `WATCH_LOST_QUEUED`, `WATCH_LOST_QUEUE_FAILED`, callback `emit_record` riuscito e callback fallito | verifica il primo confine strutturato lost-scope: il backend scrive prima il log compatibile, poi offre i record queue-level a `emit_record`; il test resta focalizzato sul momento di handoff verso la recovery ampia |
| lost-scope scan output routing | `tests/backend/test_lost_scope_scan_output_routing.sh` + `tests/backend/test_lost_scope_scan_output_routing.c` | `WATCH_LOST_SCAN_BEGIN`, `WATCH_LOST_FOUND`, `WATCH_LOST_PREFIX_UPDATED`, `WATCH_LOST_COVERAGE_DONE`, `WATCH_LOST_COVERAGE_MISSING`, `WATCH_LOST_COVERAGE_CLASS`, `WATCH_LOST_NOT_FOUND`, `WATCH_LOST_RECOVERY_FAILED`, callback `emit_record` riuscito, callback fallito, path lungo che forza fallback legacy del text sink | verifica il confine strutturato della fase di ricerca e classificazione lost-scope: old/new path, contatori di coverage, missing path, retry count ed error token vengono preservati nel record; il caso path lungo prova che il fallback di `events.log` non salta `emit_record` |
| lost-scope completion output routing | `tests/backend/test_lost_scope_completion_output_routing.sh` + `tests/backend/test_lost_scope_completion_output_routing.c` | `WATCH_LOST_REINSTALLED`, `WATCH_LOST_REINSTALL_FAILED`, `WATCH_LOST_ROLLBACK`, `WATCH_LOST_RETRY_SCHEDULED`, `WATCH_LOST_RECOVERY_GAVE_UP`, `WATCH_LOST_RECOVERY_END`, callback `emit_record` riuscito e callback fallito | verifica l'ultimo confine strutturato lost-scope: i record che installano/rimuovono watch, schedulano retry, abbandonano recovery o dichiarano `VALID` preservano `detail_path`, `related_watch_id`, `retry_count`, `delay_ms`, `pending_count`, `state` e `watches_count` |
| overflow raw bridge | `tests/backend/test_overflow_raw_bridge.sh` + `tests/backend/test_overflow_raw_bridge.c` | evento sintetico `IN_Q_OVERFLOW` con `wd=-1` trasformato in `ALFRED_RAW_OVERFLOW` con path vuoto | verifica il confine backend/core per un evento globale che non ha path nella watcher table. Non forza un overflow reale del kernel, quindi resta stabile in CI |
| overflow raw JSONL | `tests/jsonl/test_overflow_raw_jsonl.sh` + `tests/jsonl/test_overflow_raw_jsonl.c` | evento sintetico `IN_Q_OVERFLOW` con `wd=-1`, `backend_build_overflow_raw()`, `alfred_record_from_raw()`, `alfred_record_format_jsonl()`, record `RAW_OVERFLOW` con `raw_mask=128` e `path=""` | verifica il contratto pubblico strutturato per overflow senza generare un overflow reale del kernel. Il path vuoto e' parte del contratto perche' l'overflow riguarda l'intera istanza inotify, non un singolo watch |
| overflow core JSONL | `tests/jsonl/test_overflow_core_jsonl.sh` + `tests/jsonl/test_overflow_core_jsonl.c` | `alfred_raw_event_t` sintetico con `ALFRED_RAW_OVERFLOW`, `alfred_process()`, callback `ALFRED_EV_OVERFLOW`, `alfred_record_from_event()`, record JSONL `OVERFLOW` senza path | verifica il livello semantico core dell'overflow senza dipendere da timing o pressione reale della coda kernel. Il test completa il golden raw: uno controlla il bridge `IN_Q_OVERFLOW -> RAW_OVERFLOW`, l'altro controlla `RAW_OVERFLOW -> OVERFLOW` |
| lost-scope runtime recovery JSONL | `tests/jsonl/test_lost_scope_runtime_recovery_jsonl.sh` | due root configurate, directory watched spostata da root A a root B, `WATCH_STALE`, `WATCH_RESYNC_FAILED`, `WATCH_LOST_QUEUED`, scan root A, `WATCH_LOST_NOT_FOUND`, scan root B, `WATCH_LOST_FOUND`, `WATCH_LOST_RECOVERY_END`, `RAW_CREATE` e `FILE_CREATED` nel path recuperato | verifica il contratto pubblico JSONL della recovery completa. Il test distingue il `DIR_MOVED` semantico prodotto dal parent-level move pair dalla riparazione del watch figlio che ha ricevuto `IN_MOVE_SELF`; dopo la recovery Alfred deve emettere nuovi eventi sul path root B e non sul vecchio path root A stale |
| onlydir rejects file root | `tests/backend/test_onlydir_rejects_file_root.sh` + `tests/backend/test_onlydir_rejects_file_root.c` | file regolare passato a `watch_manager_add()`, errore `inotify_add_watch ... errno=20`, nessun `WATCH_ADDED` | verifica che Alfred installi watch solo su directory. I file vengono osservati tramite la directory padre, non con un watch dedicato per ogni file |
| delete self nested watch | `tests/backend/test_delete_self_nested_watch.sh` | parent `IN_DELETE | IN_ISDIR name=child`, child `IN_DELETE_SELF`, `WATCH_STALE`, `WATCH_REMOVED`, un solo `DIR_DELETED` | verifica la distinzione fra semantica e diagnostica: il delete semantico del child nasce dal parent event con nome, mentre il self-event del child aggiorna solo lo stato del watch e non accoda lost-scope recovery |
| self events root watch | `tests/backend/test_self_events_root_watch.sh` | `IN_DELETE_SELF`, `IN_IGNORED`, `IN_MOVE_SELF`, `WATCH_STALE`, `WATCH_RESYNC_BEGIN`, `WATCH_RESYNC_FAILED`, `WATCH_LOST_QUEUED`, `WATCH_STALE_EVENT_DROPPED`, `WATCH_REMOVED` e comportamento della root spostata | verifica osservativa per distinguere eventi sul path osservato da eventi sui figli; delete-self marca il watch stale e attende cleanup, move-self avvia un probe resync conservativo senza inventare delete dei figli o relocation semantica; se il vecchio path non e' raggiungibile, Alfred accoda recovery ampia ma continua a non inoltrare eventi raw/core su un path stale |
...
| self move identity mismatch | `tests/backend/test_self_move_identity_mismatch.sh` | `IN_MOVE_SELF`, path osservato ricreato, `WATCH_RESYNC_FAILED ... error=identity-mismatch`, `WATCH_LOST_QUEUED ... error=identity-mismatch`, nessun `WATCH_RESYNC_SCAN_DONE` | dimostra perche' il path da solo non basta: se la root osservata viene spostata e una nuova directory nasce nello stesso path, Alfred confronta `(st_dev, st_ino)`, non scansiona il path riusato, lascia il watch `STALE` e accoda recovery ampia tramite identita' salvata |
...
| lost scope queue | `tests/backend/test_lost_scope_queue.sh` + `tests/backend/test_lost_scope_queue.c` | helper statici `backend_lost_scope_queue_*()` senza log runtime, incluso `peek` read-only della testa FIFO, identita' numeriche opache e selezione root configurata | verifica la struttura dati che prepara la recovery posticipata: lifecycle, ordine FIFO, crescita del buffer circolare, copia stabile di path/scan_root/reason, controllo della maturita' senza pop, conservazione di `st_dev/st_ino` anche se valgono zero, rifiuto di input invalidi, deduplica root e scelta della root annidata piu' specifica |
| lost scope recovery | `tests/backend/test_lost_scope_recovery.sh` + `tests/backend/test_lost_scope_recovery.c` | `WATCH_LOST_SCAN_BEGIN`, `WATCH_LOST_FOUND`, `WATCH_LOST_PREFIX_UPDATED`, `WATCH_LOST_COVERAGE_DONE`, `WATCH_LOST_COVERAGE_MISSING`, `WATCH_LOST_COVERAGE_CLASS`, `WATCH_LOST_REINSTALLED`, `WATCH_LOST_RECOVERY_END`, `WATCH_LOST_REINSTALL_FAILED`, `WATCH_LOST_ROLLBACK`, `WATCH_LOST_RECOVERY_FAILED`, `WATCH_LOST_NOT_FOUND`, `WATCH_LOST_RETRY_SCHEDULED`, `WATCH_LOST_RECOVERY_GAVE_UP`, processore due che non consuma entry future e usa `scan_root`, failure simulata di `emit_record` su `WATCH_LOST_FOUND` | verifica la recovery sincrona completa: il ramo positivo ritrova una directory per `(st_dev, st_ino)`, riallinea path, reinstalla un missing watch e marca la subtree `VALID`; il ramo failure forza il secondo fake add a fallire, rimuove il primo fake watch installato nello stesso tentativo e lascia root/child `STALE`; una directory cancellata non viene trovata; una entry con `retry_after_ns` futuro resta in queue senza log; un `NOT_FOUND` maturo viene rischedulato con backoff e, al limite di tentativi, viene abbandonato con log diagnostico senza evento raw/core; un test passa una root chiamante volutamente sbagliata e prova che la scansione usa la `scan_root` salvata nella entry; un test sposta la directory da una root configurata a un'altra e prova che il fallback multi-root recupera prima di schedulare retry; un test usa un fd non bloccante idle per provare che `inotify_backend_poll()` processa una entry matura dal percorso runtime; un test prova che il wrapper runtime processa comunque una entry auto-contenuta con `scan_root` anche quando non ci sono root configurate per il fallback; il nuovo scenario di output failure prova che un record `WATCH_LOST_*` scritto nel log compatibile ma rifiutato da `emit_record` ferma la recovery prima di `WATCH_LOST_RECOVERY_END` |
| lost scope runtime recovery | `tests/backend/test_lost_scope_runtime_recovery.sh` | `IN_MOVE_SELF`, `WATCH_STALE`, `WATCH_RESYNC_FAILED ... error=path-unreachable`, `WATCH_LOST_QUEUED`, `WATCH_LOST_SCAN_BEGIN` su root A, `WATCH_LOST_NOT_FOUND`, `WATCH_LOST_SCAN_BEGIN` su root B, `WATCH_LOST_FOUND`, `WATCH_LOST_RECOVERY_END`, `FILE_CREATED` nel path recuperato | verifica il percorso piu' vicino all'end-to-end della recovery delayed: Alfred parte con due root configurate, una directory watched viene spostata da root A a root B, il probe locale fallisce per vecchio path non raggiungibile, la queue accoda lo scope perso, il poll runtime processa la entry, non trova l'identita' in root A, la trova in root B, aggiorna il path del watch e dimostra che il watch recuperato vede eventi successivi |
...
| Scenario | Script | Cosa controlla | Perche' serve |
| --- | --- | --- | --- |
| watcher reliability states | `tests/watcher/test_watcher_state.c` | store iniziale `VALID`, identita' `st_dev/st_ino`, `watcher_update_path()` che preserva stato/identita', `watcher_update_path_prefix()` che riscrive una subtree preservando stato/identita', ignora prefissi simili ma esterni e non corrompe sostituzioni esatte con stesso basename finale, `watcher_set_state_prefix()` che marca affidabile solo la subtree recuperata, transizioni a `STALE` e `RESYNCING`, remove a `REMOVED`, rifiuto di `REMOVED` su slot attivo, conteggio per stato, iterazione per stato e stop anticipato | prepara la futura gestione `IN_MOVE_SELF` e resync senza dipendere da timing o dettagli del kernel |

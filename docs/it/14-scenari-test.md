...
| Scenario | Script | Cosa controlla | Perche' serve |
| --- | --- | --- | --- |
| watcher reliability states | `tests/watcher/test_watcher_state.c` | store iniziale `VALID`, identita' `st_dev/st_ino`, `watcher_update_path()` che preserva stato/identita', `watcher_update_path_prefix()` che riscrive una subtree preservando stato/identita' e ignorando prefissi simili ma esterni, `watcher_set_state_prefix()` che marca affidabile solo la subtree recuperata, transizioni a `STALE` e `RESYNCING`, remove a `REMOVED`, rifiuto di `REMOVED` su slot attivo, conteggio per stato, iterazione per stato e stop anticipato | prepara la futura gestione `IN_MOVE_SELF` e resync senza dipendere da timing o dettagli del kernel |

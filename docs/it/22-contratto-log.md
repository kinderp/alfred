...
| `WATCH_LOST_SCAN_BEGIN root=R pending=K` | una recovery sincrona consuma una entry lost-scope e scansiona una root monitorata | Alfred sta cercando una identita' persa dentro `R`; `K` e' il numero di entry rimaste in coda | non significa che la directory sara' trovata; non modifica ancora prefissi o watch figli |
| `WATCH_LOST_FOUND wd=N old_path=P new_path=Q reason=R` | lo scan trova una directory con la stessa identita' salvata | Alfred ha ritrovato l'oggetto a `Q` e aggiorna il path del watch principale | non significa ancora che prefissi figli, watch mancanti o stato `VALID` siano stati aggiornati |
| `WATCH_LOST_NOT_FOUND wd=N path=P reason=R retry=K` | lo scan non trova la stessa identita' nella root analizzata | Alfred non ha recuperato lo scope in questa passata | non significa che l'oggetto non esista altrove; potrebbe essere fuori dalla root scansionata |

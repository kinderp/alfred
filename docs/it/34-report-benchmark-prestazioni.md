...
| --- | --- | --- | --- |
| Core input | `core-input-raw-first`, `raw-to-record-adapter`, `raw-to-record-plus-core` | Input raw sintetico verso `alfred_process()`, verso l'adapter `alfred_record_from_raw()` o verso entrambi in sequenza | Baseline per futuri bridge record/raw o core record-first; non confrontare direttamente con JSONL o runtime reale |
...
| `output_mode` | Diagnostico core-input | Modalita' usata dal benchmark core input. `counter-noop` indica callback semantica senza I/O; `adapter-only` indica conversione senza callback semantica; `counter-noop+adapter` indica conversione raw->record piu' core raw-first. |
...
| `raw-to-record-plus-core` | `alfred_raw_event_t -> alfred_record_from_raw() -> alfred_process() -> callback counter/no-op` | Costo di produrre un record normalizzato come sidecar mantenendo il core raw-first |
...
core-input-raw-first,100000,5,165194,181274.20,204364,551650.48,90000,0,0,counter-noop
raw-to-record-adapter,100000,5,3624,4754.00,5675,21034917.96,0,0,1,adapter-only
raw-to-record-plus-core,100000,5,189744,228227.60,292305,438159.10,90000,0,1,counter-noop+adapter
```
...
- `conversions_per_event` deve restare `0` per la baseline raw-first e `1`
  per le righe che chiamano `alfred_record_from_raw()`;
- `output_mode` deve restare `counter-noop` per il core, `adapter-only` per
  la conversione isolata e `counter-noop+adapter` per la riga combinata;
...
- la callback ha visto `90000` eventi semantici perche' ogni blocco da dieci
  ...
  un evento semantico;
- `errors_last=0` rende valide le righe del refresh;
...
- `raw_events_per_sec_avg=551650.48` per `core-input-raw-first` e'
  orientativo su questa VM con
  `runs=5`, non una soglia CI e non una promessa di throughput runtime reale.
- `raw_events_per_sec_avg=21034917.96` per `raw-to-record-adapter` indica che la
  ...
  core;
- `raw-to-record-plus-core` resta nello stesso ordine di grandezza della riga
  raw-first, ma in questo refresh e' piu' rumoroso (`min_us=189744`,
  `max_us=292305`). Questo suggerisce che produrre un record normalizzato
  sidecar e poi alimentare il core raw-first non appare catastrofico in questa
  micro-misura, ma non basta per fissare una soglia o accettare un bridge
  completo;
- la distanza tra `min_us=165194` e `max_us=204364` per la riga core mostra gia'
  rumore di misura; per una soglia futura serviranno run ripetuti e ambiente
  piu' controllato.

...
# - normalized raw RAW_CREATE records exist for src, dst and src/item with
#   raw_mask=257
# - normalized raw RAW_MOVED_FROM exists for src/item with raw_mask=288
# - normalized raw RAW_MOVED_TO exists for dst/item with raw_mask=320
# - RAW_MOVED_FROM and RAW_MOVED_TO expose a non-zero matching cookie
# - exactly one semantic DIR_MOVED record exists with old_path=src/item and
#   new_path=dst/item
# - no semantic DIR_RENAMED or DIR_RELOCATED record exists for this operation
#
# Compatibility logs expected in parallel:
# - raw.log still contains RAW_CREATE, RAW_MOVED_FROM and RAW_MOVED_TO
# - events.log still contains DIR_CREATED and DIR_MOVED
# - events.log must not contain DIR_RENAMED or DIR_RELOCATED for this operation
#
# Meaning:
# This golden test fixes the structured contract for moving a directory to a
...
TEST_ROOT="${TEST_ROOT:-/tmp/alfred_jsonl_test_dir_moved}"
ALFRED_BIN="${ALFRED_BIN:-../../alfred}"
CONFIG_FILE="./jsonl-dir-moved.conf"
OUTPUT_LOG="./output.jsonl"
ALFRED_PID=""

source ../core/test_lib.sh

cleanup_jsonl_dir_moved() {
    stop_alfred
    rm -rf "$TEST_ROOT"
...
trap cleanup_jsonl_dir_moved EXIT
...
if ! grep -Eq "RAW_CREATE path=.*/src/item mask=257" ./raw.log; then
    fail_with_all_logs "missing compatibility RAW_CREATE for src/item"
fi

if ! grep -Eq "RAW_MOVED_FROM path=.*/src/item mask=288 cookie=[1-9][0-9]*" ./raw.log; then
    fail_with_all_logs "missing compatibility RAW_MOVED_FROM for src/item"
fi

if ! grep -Eq "RAW_MOVED_TO path=.*/dst/item mask=320 cookie=[1-9][0-9]*" ./raw.log; then
    fail_with_all_logs "missing compatibility RAW_MOVED_TO for dst/item"
fi

if ! grep -Eq "DIR_CREATED path=.*/src/item" ./events.log; then
    fail_with_all_logs "missing compatibility DIR_CREATED for src/item"
fi

if ! grep -Eq "DIR_MOVED from=.*/src/item to=.*/dst/item" ./events.log; then
    fail_with_all_logs "missing compatibility DIR_MOVED for move"
fi

if grep -Eq "^DIR_RENAMED " ./events.log; then
    fail_with_all_logs "move unexpectedly produced DIR_RENAMED"
fi

if grep -Eq "^DIR_RELOCATED " ./events.log; then
    fail_with_all_logs "move unexpectedly produced DIR_RELOCATED"
fi
...
src_path = f"{root}/src"
dst_path = f"{root}/dst"
old_path = f"{root}/src/item"
new_path = f"{root}/dst/item"
...
wrong_semantics = [
    ...
    if record.get("layer") == "semantic"
    and record.get("category") == "filesystem"
    and record.get("type") in {"DIR_RENAMED", "DIR_RELOCATED"}
]
if wrong_semantics:
    raise SystemExit(
        "directory move produced unexpected DIR_RENAMED/DIR_RELOCATED "
...
echo "PASS jsonl golden directory move"

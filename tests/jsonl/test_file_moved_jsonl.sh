...
# - normalized raw RAW_CREATE records exist for src, dst and src/item.txt
# - directory creates use raw_mask=257, while the file create uses raw_mask=1
# - normalized raw RAW_MOVED_FROM exists for src/item.txt with raw_mask=32
# - normalized raw RAW_MOVED_TO exists for dst/item.txt with raw_mask=64
# - RAW_MOVED_FROM and RAW_MOVED_TO expose a non-zero matching cookie
# - exactly one semantic FILE_MOVED record exists with old_path=src/item.txt and
#   new_path=dst/item.txt
# - no semantic FILE_RENAMED or FILE_RELOCATED record exists for this operation
#
# Compatibility logs expected in parallel:
# - raw.log still contains RAW_CREATE, RAW_MOVED_FROM and RAW_MOVED_TO
# - events.log still contains FILE_CREATED and FILE_MOVED
# - events.log must not contain FILE_RENAMED or FILE_RELOCATED for this
#   operation
#
# Meaning:
# This golden test fixes the structured contract for moving a file to a
# different parent while keeping the same basename. At the core semantic level,
# changing only the parent directory is a move, not a rename and not a
# relocation.

set -euo pipefail

TEST_ROOT="${TEST_ROOT:-/tmp/alfred_jsonl_test_file_moved}"
ALFRED_BIN="${ALFRED_BIN:-../../alfred}"
CONFIG_FILE="./jsonl-file-moved.conf"
OUTPUT_LOG="./output.jsonl"
ALFRED_PID=""

source ../core/test_lib.sh

cleanup_jsonl_file_moved() {
    stop_alfred
    rm -rf "$TEST_ROOT"
...
trap cleanup_jsonl_file_moved EXIT
...
if ! grep -Eq "RAW_CREATE path=.*/src/item.txt mask=1" ./raw.log; then
    fail_with_all_logs "missing compatibility RAW_CREATE for src/item.txt"
fi

if ! grep -Eq "RAW_MOVED_FROM path=.*/src/item.txt mask=32 cookie=[1-9][0-9]*" ./raw.log; then
    fail_with_all_logs "missing compatibility RAW_MOVED_FROM for src/item.txt"
fi

if ! grep -Eq "RAW_MOVED_TO path=.*/dst/item.txt mask=64 cookie=[1-9][0-9]*" ./raw.log; then
    fail_with_all_logs "missing compatibility RAW_MOVED_TO for dst/item.txt"
fi

if ! grep -Eq "FILE_CREATED path=.*/src/item.txt" ./events.log; then
    fail_with_all_logs "missing compatibility FILE_CREATED for src/item.txt"
fi

if ! grep -Eq "FILE_MOVED from=.*/src/item.txt to=.*/dst/item.txt" ./events.log; then
    fail_with_all_logs "missing compatibility FILE_MOVED for move"
fi

if grep -Eq "^FILE_RENAMED " ./events.log; then
    fail_with_all_logs "move unexpectedly produced FILE_RENAMED"
fi

if grep -Eq "^FILE_RELOCATED " ./events.log; then
    fail_with_all_logs "move unexpectedly produced FILE_RELOCATED"
fi
...
src_path = f"{root}/src"
dst_path = f"{root}/dst"
old_path = f"{root}/src/item.txt"
new_path = f"{root}/dst/item.txt"
...
moves = [
    ...
    and record.get("type") == "FILE_MOVED"
    and record.get("old_path") == old_path
    and record.get("new_path") == new_path
]
if len(moves) != 1:
    raise SystemExit(
        "expected exactly one semantic FILE_MOVED record with "
        f"expected old_path/new_path, found {len(moves)}"
    )
...
wrong_semantics = [
    ...
    if record.get("layer") == "semantic"
    and record.get("category") == "filesystem"
    and record.get("type") in {"FILE_RENAMED", "FILE_RELOCATED"}
]
if wrong_semantics:
    raise SystemExit(
        "file move produced unexpected FILE_RENAMED/FILE_RELOCATED "
...
echo "PASS jsonl golden file move"

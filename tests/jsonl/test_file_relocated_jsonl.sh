...
# - normalized raw RAW_CREATE records exist for src, dst and src/before.txt
# - directory creates use raw_mask=257, while the file create uses raw_mask=1
# - normalized raw RAW_MOVED_FROM exists for src/before.txt with raw_mask=32
# - normalized raw RAW_MOVED_TO exists for dst/after.txt with raw_mask=64
# - RAW_MOVED_FROM and RAW_MOVED_TO expose a non-zero matching cookie
# - exactly one semantic FILE_RELOCATED record exists with
#   old_path=src/before.txt and new_path=dst/after.txt
# - no semantic FILE_MOVED or FILE_RENAMED record exists for this operation
#
# Compatibility logs expected in parallel:
# - raw.log still contains RAW_CREATE, RAW_MOVED_FROM and RAW_MOVED_TO
# - events.log still contains FILE_CREATED and FILE_RELOCATED
# - events.log must not contain FILE_MOVED or FILE_RENAMED for this operation
#
# Meaning:
# This golden test fixes the structured contract for moving and renaming a file
# in one operation. At the core semantic level, changing both parent directory
# and basename is a relocation, not a pure move and not a pure rename.

set -euo pipefail

TEST_ROOT="${TEST_ROOT:-/tmp/alfred_jsonl_test_file_relocated}"
ALFRED_BIN="${ALFRED_BIN:-../../alfred}"
CONFIG_FILE="./jsonl-file-relocated.conf"
OUTPUT_LOG="./output.jsonl"
ALFRED_PID=""

source ../core/test_lib.sh

cleanup_jsonl_file_relocated() {
    stop_alfred
    rm -rf "$TEST_ROOT"
...
trap cleanup_jsonl_file_relocated EXIT
...
if ! grep -Eq "RAW_CREATE path=.*/src/before.txt mask=1" ./raw.log; then
    fail_with_all_logs "missing compatibility RAW_CREATE for src/before.txt"
fi

if ! grep -Eq "RAW_MOVED_FROM path=.*/src/before.txt mask=32 cookie=[1-9][0-9]*" ./raw.log; then
    fail_with_all_logs "missing compatibility RAW_MOVED_FROM for src/before.txt"
fi

if ! grep -Eq "RAW_MOVED_TO path=.*/dst/after.txt mask=64 cookie=[1-9][0-9]*" ./raw.log; then
    fail_with_all_logs "missing compatibility RAW_MOVED_TO for dst/after.txt"
fi

if ! grep -Eq "FILE_CREATED path=.*/src/before.txt" ./events.log; then
    fail_with_all_logs "missing compatibility FILE_CREATED for src/before.txt"
fi

if ! grep -Eq "FILE_RELOCATED from=.*/src/before.txt to=.*/dst/after.txt" ./events.log; then
    fail_with_all_logs "missing compatibility FILE_RELOCATED for relocation"
fi

if grep -Eq "^FILE_MOVED " ./events.log; then
    fail_with_all_logs "relocation unexpectedly produced FILE_MOVED"
fi

if grep -Eq "^FILE_RENAMED " ./events.log; then
    fail_with_all_logs "relocation unexpectedly produced FILE_RENAMED"
fi
...
src_path = f"{root}/src"
dst_path = f"{root}/dst"
old_path = f"{root}/src/before.txt"
new_path = f"{root}/dst/after.txt"
...
raw_create_file = find_record(
    "normalized_raw", "filesystem", "RAW_CREATE", old_path
)
if raw_create_file.get("raw_mask") != 1:
    raise SystemExit(
        f"RAW_CREATE path={old_path} raw_mask={raw_create_file.get('raw_mask')!r}"
    )
...
relocations = [
    ...
    and record.get("type") == "FILE_RELOCATED"
    and record.get("old_path") == old_path
    and record.get("new_path") == new_path
]
if len(relocations) != 1:
    raise SystemExit(
        "expected exactly one semantic FILE_RELOCATED record with "
...
wrong_semantics = [
    ...
    if record.get("layer") == "semantic"
    and record.get("category") == "filesystem"
    and record.get("type") in {"FILE_MOVED", "FILE_RENAMED"}
]
if wrong_semantics:
    raise SystemExit(
        "file relocation produced unexpected FILE_MOVED/FILE_RENAMED "
...
echo "PASS jsonl golden file relocation"

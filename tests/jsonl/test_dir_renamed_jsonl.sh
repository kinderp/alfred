...
# - one normalized raw RAW_CREATE record exists for old-dir with raw_mask=257
# - normalized raw RAW_MOVED_FROM exists for old-dir with raw_mask=288
# - normalized raw RAW_MOVED_TO exists for new-dir with raw_mask=320
# - RAW_MOVED_FROM and RAW_MOVED_TO expose a non-zero matching cookie
# - exactly one semantic DIR_RENAMED record exists with old_path=old-dir and
#   new_path=new-dir
# - no semantic DIR_MOVED or DIR_RELOCATED record exists for this operation
#
# Compatibility logs expected in parallel:
# - raw.log still contains RAW_CREATE, RAW_MOVED_FROM and RAW_MOVED_TO
# - events.log still contains DIR_CREATED and DIR_RENAMED
# - events.log must not contain DIR_MOVED or DIR_RELOCATED for this operation
#
# Meaning:
# This golden test fixes the structured contract for renaming a directory
# inside the same parent directory. At the core semantic level, changing only
# the basename is a rename, not a move and not a relocation.

set -euo pipefail

TEST_ROOT="${TEST_ROOT:-/tmp/alfred_jsonl_test_dir_renamed}"
ALFRED_BIN="${ALFRED_BIN:-../../alfred}"
CONFIG_FILE="./jsonl-dir-renamed.conf"
OUTPUT_LOG="./output.jsonl"
ALFRED_PID=""

source ../core/test_lib.sh

cleanup_jsonl_dir_renamed() {
    stop_alfred
    rm -rf "$TEST_ROOT"
...
trap cleanup_jsonl_dir_renamed EXIT
...
if ! grep -Eq "RAW_CREATE path=.*/old-dir mask=257" ./raw.log; then
    fail_with_all_logs "missing compatibility RAW_CREATE for old-dir"
fi

if ! grep -Eq "RAW_MOVED_FROM path=.*/old-dir mask=288 cookie=[1-9][0-9]*" ./raw.log; then
    fail_with_all_logs "missing compatibility RAW_MOVED_FROM for old-dir"
fi

if ! grep -Eq "RAW_MOVED_TO path=.*/new-dir mask=320 cookie=[1-9][0-9]*" ./raw.log; then
    fail_with_all_logs "missing compatibility RAW_MOVED_TO for new-dir"
fi

if ! grep -Eq "DIR_CREATED path=.*/old-dir" ./events.log; then
    fail_with_all_logs "missing compatibility DIR_CREATED for old-dir"
fi

if ! grep -Eq "DIR_RENAMED from=.*/old-dir to=.*/new-dir" ./events.log; then
    fail_with_all_logs "missing compatibility DIR_RENAMED for rename"
fi

if grep -Eq "^DIR_MOVED " ./events.log; then
    fail_with_all_logs "rename unexpectedly produced DIR_MOVED"
fi

if grep -Eq "^DIR_RELOCATED " ./events.log; then
    fail_with_all_logs "rename unexpectedly produced DIR_RELOCATED"
fi
...
root = sys.argv[2]
old_path = f"{root}/old-dir"
new_path = f"{root}/new-dir"
...
renames = [
    ...
    and record.get("type") == "DIR_RENAMED"
    and record.get("old_path") == old_path
    and record.get("new_path") == new_path
]
if len(renames) != 1:
    raise SystemExit(
        "expected exactly one semantic DIR_RENAMED record with "
        f"expected old_path/new_path, found {len(renames)}"
    )
...
wrong_semantics = [
    ...
    if record.get("layer") == "semantic"
    and record.get("category") == "filesystem"
    and record.get("type") in {"DIR_MOVED", "DIR_RELOCATED"}
]
if wrong_semantics:
    raise SystemExit(
        "directory rename produced unexpected DIR_MOVED/DIR_RELOCATED "
...
echo "PASS jsonl golden directory rename"
